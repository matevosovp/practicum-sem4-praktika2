...
- **Recall@10: 0.1496** у итоговой модели против **0.0085** у глобального popularity baseline.
- Пайплайн обрабатывает более **20 млн строк** свойств товаров без построения dense user-item матрицы.
- Реализован полный контур: offline evaluation → MLflow → PostgreSQL ETL → FastAPI → Prometheus → Airflow retraining.
- Разбиение по времени защищает оценку от temporal leakage.
...
EDA оформлен в [notebooks/01_eda.ipynb](notebooks/01_eda.ipynb) как самостоятельный review-ready артефакт. Ключевые инженерные выводы:
...
Результаты зафиксированы в [artifacts/reports/evaluation_summary.json](artifacts/reports/evaluation_summary.json).
...
- [models/recommender.joblib](models/recommender.joblib)

Дополнительный serving-артефакт:

- [models/serving_history.parquet](models/serving_history.parquet)
...
scripts/                 CLI для EDA, train, reporting и MLflow
...
python scripts/export_notebook_reports.py
```

`notebooks/01_eda.ipynb` и `notebooks/02_modeling.ipynb` поддерживаются вручную как финальные документы проекта и используют уже подготовленные parquet/json artifacts. Скрипт `export_notebook_reports.py` обновляет только компактные вспомогательные отчеты для modeling/EDA секций, но не генерирует ноутбуки.
...
Подробное описание слоев есть в [docs/etl.md](docs/etl.md).
...
Docker-образ использует уже сериализованную модель из каталога `models/`. Корневой `Dockerfile` предназначен для API; Airflow собирается отдельно через `Dockerfile.airflow`, указанный в `docker-compose.yaml`.
...
```bash
cp .env.example .env
# при необходимости заполните S3-параметры в .env
...
- [airflow/dags/retrain_recommender.py](airflow/dags/retrain_recommender.py)
- [airflow/dags/recsys_etl_to_postgres.py](airflow/dags/recsys_etl_to_postgres.py)
...
Подробности описаны в [docs/monitoring.md](docs/monitoring.md).
...
- `time-based split` жёстко зашит в pipeline
- промежуточные артефакты сохраняются на диск
- модель и serving history сериализуются отдельно
- ноутбуки являются самостоятельными артефактами и используют уже рассчитанные parquet/json-данные, без обязательного повторного тяжёлого preprocessing при открытии
- компактные notebook-specific отчеты можно обновить через `python scripts/export_notebook_reports.py`
- Airflow не генерирует notebooks; вместо этого он сохраняет [artifacts/reports/reporting_metrics.json](artifacts/reports/reporting_metrics.json) как компактный слой метрик для последующего анализа
